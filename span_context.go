...
func (c SpanContext) String() string {
	if c.traceID.High == 0 {
		return fmt.Sprintf("%016x:%016x:%x:%x", c.traceID.Low, uint64(c.spanID), uint64(c.parentID), c.samplingState.stateFlags.Load())
	}
	return fmt.Sprintf("%016x%016x:%016x:%x:%x", c.traceID.High, c.traceID.Low, uint64(c.spanID), uint64(c.parentID), c.samplingState.stateFlags.Load())
}
